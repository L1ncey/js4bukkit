package org.js4bukkit.script.interop.command.utils;
...
public class CommandUtils {
    ...
    public static void unregister(@NonNull Command command) {
        ...
        knownCommands.entrySet().removeIf(
                entry -> entry.getKey().equalsIgnoreCase(name) || entry.getKey().startsWith(name + ":") || aliases.contains(entry.getKey())
        );
    }
    ...
    public static Map<String, Command> getKnownCommands(CommandMap commandMap) {
        if (Js4Bukkit.getServerVersion() >= 1013) {
